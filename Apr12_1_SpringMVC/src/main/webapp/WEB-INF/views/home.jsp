...
<h1>
	...
	package명이 같으면 프로젝트가 달라져도 주소가 같아질 수 있어서
	에러 발생의 확률이 높으니 top-level package는 이름이 중복되지 않도록 주의 !
	
	Spring 프로그램 실행 -> 프로젝트 자체를 선택 후 Ctrl + F11 -> Run on Server
	
	Spring 이전에
	애플리케이션 구현 -> EJB(Enterprise JavaBeans)
	EJB의 기술이 점점 발전하면서 성능이 그만큼 느려지게 되고,
	기능이 추가할수록 더욱더 사용이 점점 더 복잡해지게 되었는데...
	...
	- src - main - webapp - WEB-INF 폴더
		...
		webapp - resources 폴더				: .css, .js, img들, ...
		...
		web.xml : 프로그램 실행시 함께 따라가는 설정들을 모아둔 파일
			클라이언트의 요청을 처리해주는 [DispatcherServlet],
			웹 애플리케이션의 단위 설정을 불러오는 [ContextLoaderListener],
